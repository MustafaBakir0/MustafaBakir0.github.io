<!-- index.html -->
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Mustafa Bakir | Multimedia Designer</title>
    <link rel="stylesheet" href="styles.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/p5.js/1.6.0/p5.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/mathjax/3.2.0/es5/tex-mml-chtml.js"></script>
</head>
<body>
    <header>
        <nav>
            <ul>
                <li><a href="#work">Work</a></li>
                <li><a href="#about">About</a></li>
                <li><a href="#contact">Contact</a></li>
                <li><a href="https://calendly.com/mb9457-nyu/30min" target="_blank" class="calendly-link">Schedule a Meeting</a></li>
            </ul>
        </nav>
    </header>

    <section class="hero">
        <div class="content">
            <h1 class="main-title">Mustafa Bakir</h1>
            <div class="typewriter">
                <span class="dynamic-text"></span>
            </div>
        </div>
    </section>

    <!-- Wrap all content after hero in a container with margin-top -->
    <div class="content-container">
        <!-- Work section -->
        <section id="work" class="section">
            <h2>Featured Projects</h2>
            <div class="projects-grid">
                <!-- Project items (no changes) -->
                <div class="project-item" data-project="project1">
                    <img src="assets/this.jpg" alt="Project 1">
                    <div class="project-overlay">
                        <h3 class="project-title">Library Catalog Management System (LCMS)</h3>
                    </div>
                </div>
                
                <div class="project-item" data-project="project2">
                    <img src="assets/DFT.jpg" alt="Project 2">
                    <div class="project-overlay">
                        <h3 class="project-title">Drawing with Fourier Transform, Polygon Guesser</h3>
                    </div>
                </div>

                <div class="project-item" data-project="project3">
                    <img src="assets/webster.png" alt="Project 3">
                    <div class="project-overlay">
                        <h3 class="project-title">Webster!</h3>
                    </div>
                </div>
            </div>
    
    <!-- Project Modals -->
    <div id="project1-modal" class="project-modal">
        <div class="modal-content">
            <button class="close-modal">&times;</button>
            <h3 class="modal-title">Library Catalog Management System (LCMS)</h3>
            <p>The Library Catalog Management System (LCMS) is a C++ application designed to efficiently organize and manage books, categories, and borrowers in a library setting. Built with object-oriented principles, this system allows books to be stored in a hierarchical category structure (e.g., Fiction/Sci-Fi or Non-Fiction/History), enabling intuitive navigation and management. The program supports comprehensive book operations, including adding, editing, removing, and searching for books by title, as well as importing and exporting catalog data via CSV files.</p>
            <p>At its core, the system leverages a custom tree data structure to maintain the category hierarchy, with each node tracking its book count and subcategories. For dynamic storage, I implemented MyVector, a custom vector class with iterator support, dynamic resizing, and efficient element management. The application also handles borrower management, tracking current and past book loans while providing functionalities for borrowing, returning, and listing borrowed books per user.</p>
            <p>This project highlights my proficiency in C++ memory management, custom data structure design, and algorithmic problem-solving. The use of destructors for resource cleanup, operator overloading for book comparisons, and recursive tree traversal demonstrates strong software en gineering practices. The command-line interface ensures ease of use, making it a practical solution for library catalog management.</p>
            <p>Designed to showcase my ability to develop structured, scalable systems, this project is an excellent addition to my portfolio, emphasizing C++ expertise, efficient data organization, and real-world application development.</p>
            <a href="https://github.com/MustafaBakir0/Library-Catalog-Management-System-LCMS-" class="github-link" target="_blank"><i class="fab fa-github"></i> View on GitHub</a>
        </div>
    </div>
    
    <div id="project2-modal" class="project-modal">
        <div class="modal-content">
            <button class="close-modal">&times;</button>
            <h3 class="modal-title">Drawing with Fourier Transform, Polygon Guesser</h3>

            <div id="project2-sketch-container">
        </div>
  
  <p>This project explores the fascinating intersection of mathematics and art through Fourier transformations. The implementation allows users to draw shapes that are then reconstructed using a series of rotating circles (epicycles) derived from Fourier analysis.</p>
  
  <div id="project2-sketch-container">
    <iframe src="https://editor.p5js.org/mb9457/sketches/4DY5y8UJI" width="600" height="640" frameborder="0"></iframe>
  </div>
  
  <h3>Fourier Epicycle Reconstruction: The Math Behind the Animation</h3>
  
  <p>The animation above demonstrates how any drawing can be approximated by a sum of rotating circles. This is achieved through four key algorithms:</p>
  
  <div class="algorithm-section">
    <h4>1. Resample Function</h4>
    <div class="algorithm-description">
      <p>This function resamples an array of points to have \( n \) evenly spaced points along the path, ensuring uniform sampling for the Fourier transform.</p>
    </div>
    
    <div class="algorithm-io">
      <p><strong>Input:</strong></p>
      <ul>
        <li>\(\texttt{points[]}\): Array of points (e.g. p5.js vectors)</li>
        <li>\( n \): Number of points to resample to</li>
      </ul>
      
      <p><strong>Output:</strong></p>
      <ul>
        <li>\(\texttt{newPoints[]}\): Resampled array of \( n \) evenly spaced points</li>
      </ul>
    </div>
    
    <div class="algorithm-code">
      Function Resample(points[], n):</span>
          If length(points) < 2 then</span>
              Return points     <span class="algorithm-comment">// Fewer than 2 points, return original array</span></span>
          totalLength ← 0</span>
          For i ← 1 to length(points) - 1 do</span>
              totalLength ← totalLength + Distance(points[i-1], points[i])</span>
          interval ← totalLength/(n-1)</span>
          newPoints ← [points[0]]</span>
          d ← 0</span>
          For i ← 1 to length(points) - 1 do</span>
              prev ← points[i-1]</span>
              curr ← points[i]</span>
              segmentDist ← Distance(prev, curr)</span>
              While d + segmentDist ≥ interval do</span>
                  t ← (interval - d)/segmentDist</span>
                  nx ← Lerp(prev.x, curr.x, t)</span>
                  ny ← Lerp(prev.y, curr.y, t)</span>
                  newPoint ← CreateVector(nx, ny)</span>
                  Append newPoint to newPoints</span>
                  prev ← newPoint</span>
                  segmentDist ← Distance(prev, curr)</span>
                  d ← 0</span>
              d ← d + segmentDist</span>
          Return newPoints</span>
    </div>
  </div>
  
  <div class="algorithm-section">
    <h4>2. Discrete Fourier Transform (DFT) Function</h4>
    <div class="algorithm-description">
      <p>Computes the Discrete Fourier Transform (DFT) of an array of complex numbers, adjusting the frequency for indices greater than \(N/2\).</p>
    </div>
    
    <div class="algorithm-io">
      <p><strong>Input:</strong></p>
      <ul>
        <li>\(\texttt{x[]}\): Array of complex numbers represented by objects \(\{ \text{re}, \text{im} \}\) (real and imaginary parts)</li>
      </ul>
      
      <p><strong>Output:</strong></p>
      <ul>
        <li>\(\texttt{X[]}\): Array of DFT coefficients with frequency, amplitude, and phase information</li>
      </ul>
    </div>
    
    <div class="math-container">
      <p>Recall that the Fourier coefficient for index \( k \) is given by:</p>
      <p>\[
      X[k] = \frac{1}{N} \sum_{n=0}^{N-1} x[n]\, e^{-i\, \frac{2\pi k n}{N}},
      \]</p>
      
      <p>which in real/imaginary parts becomes:</p>
      <p>\[
      \begin{aligned}
      \text{re} &= \frac{1}{N} \sum_{n=0}^{N-1} \Bigl( x[n].\text{re} \cdot \cos\left(\frac{2\pi k n}{N}\right) + x[n].\text{im} \cdot \sin\left(\frac{2\pi k n}{N}\right) \Bigr), \\
      \text{im} &= \frac{1}{N} \sum_{n=0}^{N-1} \Bigl( -x[n].\text{re} \cdot \sin\left(\frac{2\pi k n}{N}\right) + x[n].\text{im} \cdot \cos\left(\frac{2\pi k n}{N}\right) \Bigr).
      \end{aligned}
      \]</p>
    </div>
    
    <div class="algorithm-code">
      Function DFT(x[]):</span>
          N ← length(x)</span>
          Initialize X[] ← {}</span>
          For k ← 0 to N-1 do</span>
              re ← 0</span>
              im ← 0</span>
              For n ← 0 to N-1 do</span>
                  φ ← (2π × k × n)/N</span>
                  re ← re + x[n].re×cos(φ) + x[n].im×sin(φ)</span>
                  im ← im - x[n].re×sin(φ) + x[n].im×cos(φ)</span>
              re ← re/N</span>
              im ← im/N</span>
              amp ← √(re² + im²)</span>
              phase ← atan2(im, re)</span>
              freq ← k</span>
              If k > N/2 then</span>
                  freq ← k - N</span>
              Append { re, im, freq, amp, phase } to X[]</span>
          Return X[]</span>
    </div>
  </div>
  
  <div class="algorithm-section">
    <h4>3. Epicycle Class</h4>
    <div class="algorithm-description">
      <p>Represents one epicycle for Fourier reconstruction, calculates its position at a given time, and displays the corresponding circle.</p>
    </div>
    
    <div class="algorithm-io">
      <p><strong>Input:</strong></p>
      <ul>
        <li>\(\texttt{time}\): Current time</li>
        <li>\(\texttt{coeff}\): Fourier coefficient containing \(\text{freq}\), \(\text{amp}\), and \(\text{phase}\)</li>
      </ul>
      
      <p><strong>Output:</strong></p>
      <ul>
        <li>\(\texttt{nextPosition}\): New position of the epicycle based on the time</li>
      </ul>
    </div>
    
    <div class="algorithm-code">
      Class Epicycle:</span>
          Constructor(x, y, coeff):</span>
              this.x ← x      <span class="algorithm-comment">// Center x-coordinate</span></span>
              this.y ← y      <span class="algorithm-comment">// Center y-coordinate</span></span>
              this.freq ← coeff.freq    <span class="algorithm-comment">// Frequency from coefficient</span></span>
              this.amp ← coeff.amp × scaleFactor    <span class="algorithm-comment">// Amplitude with scaling</span></span>
              this.phase ← coeff.phase  <span class="algorithm-comment">// Phase from coefficient</span></span>
          </span>
          Update(time):</span>
              angle ← this.phase + 2π × this.freq × time</span>
              dx ← this.amp × cos(angle)</span>
              dy ← this.amp × sin(angle)</span>
              nextPosition ← CreateVector(this.x + dx, this.y + dy)</span>
              Return nextPosition</span>
          </span>
          Display():</span>
              SetStroke(255, 100)    <span class="algorithm-comment">// Set stroke color with transparency</span></span>
              NoFill()</span>
              DrawCircle(this.x, this.y, 2 × this.amp)    <span class="algorithm-comment">// Draw epicycle circle</span></span>
    </div>
  </div>
  
  <div class="algorithm-section">
    <h4>4. Main Loop for Drawing Fourier Reconstruction</h4>
    <div class="algorithm-description">
      <p>Iterates through the Fourier coefficients, draws the epicycles, and reconstructs the shape over time. Updates the drawing path and the time variable.</p>
    </div>
    
    <div class="algorithm-io">
      <p><strong>Input:</strong></p>
      <ul>
        <li>\(\texttt{time}\): Time variable incremented each frame</li>
        <li>\(\texttt{numEpicycles}\): Number of epicycles to use for reconstruction</li>
      </ul>
      
      <p><strong>Output:</strong></p>
      <ul>
        <li>Draws the Fourier reconstruction path on the canvas</li>
      </ul>
    </div>
    
    <div class="algorithm-code">
      Function Draw():</span>
          ClearCanvas(0)    <span class="algorithm-comment">// Clear the canvas with black</span></span>
          If drawingDone then</span>
              prev ← CreateVector(width/2, height/2)    <span class="algorithm-comment">// Start at canvas center</span></span>
              For i ← 0 to numEpicycles - 1 do</span>
                  epi ← CreateEpicycle(prev.x, prev.y, fourier[i])</span>
                  epi.Display()</span>
                  next ← epi.Update(time)</span>
                  SetStroke(255)    <span class="algorithm-comment">// White stroke</span></span>
                  DrawLine(prev.x, prev.y, next.x, next.y)</span>
                  prev ← next</span>
              Append prev to path[]</span>
              If length(path) > 1000 then</span>
                  RemoveLast(path)    <span class="algorithm-comment">// Remove the oldest point in the path</span></span>
              SetStroke(255, 0, 255)    <span class="algorithm-comment">// Magenta for reconstruction path</span></span>
              NoFill()</span>
              BeginShape()</span>
              For each point in path do</span>
                  AddVertex(point.x, point.y)</span>
              EndShape()</span>
              </span>
              time ← time + dt</span>
              If time > 2π then</span>
                  time ← 0</span>
                  path ← []    <span class="algorithm-comment">// Clear the reconstruction path</span></span>
    </div>
  </div>
  
  <p>The Fourier transform decomposes a drawing into a series of rotating circles (epicycles) of different frequencies, amplitudes, and phases. When these circles are combined, they reconstruct the original drawing. The more epicycles used, the more accurate the reconstruction.</p>
  
  <p>This mathematical approach allows for fascinating animations where complex shapes emerge from simple circular motions - a beautiful demonstration of how mathematics can describe artistic forms.</p>
  
            <a href="https://github.com/yourusername/project2" class="github-link" target="_blank"><i class="fab fa-github"></i> View on GitHub</a>
        </div>
    </div>


      <div id="project3-modal" class="project-modal">
    <div class="modal-content">
        <button class="close-modal">&times;</button>
        <h3 class="modal-title">Webster - Interactive Web Game</h3>
        <div id="webster-sketch-container">
            <iframe src="https://editor.p5js.org/mb9457/full/tAD9-naTT" width="640" height="520" frameborder="0"></iframe>
        </div>
        <p>
            <h3>Webster: A Physics-Based Rope Swinging Game</h3>
            <p>
 <section id="webster-game" style="padding: 2rem; font-family: sans-serif;">
  <h2>Webster: A Physics-Based Rope Swinging Game</h2>
  <p>
    <strong>Webster</strong> is a creative 2D physics-based game inspired by a real-life spider that became an unexpected companion. Designed with a whimsical yet technical approach, Webster simulates realistic gravity and rope dynamics to let players swing through an atmospheric cave filled with clouds, collectibles, and enemies.
  </p>

  <h3>Project Highlights</h3>
  <ul>
    <li>Real-time physics engine using vector math for gravity and rope tension</li>
    <li>Custom damping, collision detection, and tension calculations</li>
    <li>Dynamic world generation with randomized spawning of obstacles, collectibles (flies, web power-ups), and enemies (bees)</li>
    <li>Modular code structure using object-oriented design principles</li>
    <li>Responsive character movement for a natural swinging experience</li>
    <li>Parallax background for added depth and immersion</li>
  </ul>

  <h3>Technology Stack</h3>
  <ul>
    <li><strong>Language:</strong> JavaScript (p5.js)</li>
    <li><strong>Framework:</strong> p5.js for rendering, physics, and input handling</li>
    <li><strong>Development Approach:</strong> Modular functions and classes to maintain scalability</li>
  </ul>

  <h3>Code Preview</h3>
  <pre><code>
// Gravity and rope physics
this.vel.add(gravity);
if (this.attached) {
  let ropeVec = p5.Vector.sub(ropeAnchor, this.pos);
  let distance = ropeVec.mag();
  if (distance > ropeRestLength) {
    let force = ropeVec.normalize().mult((distance - ropeRestLength) * ropeK);
    this.vel.add(force);
  }
}
this.vel.mult(damping);
this.pos.add(this.vel);
  </code></pre>

  <h3>Creative Intent</h3>
  <p>
    The game blends storytelling with interactive physics, turning a simple spider into a playful protagonist in a whimsical, cloud-filled cave. The random generation of clouds, flies, and bees ensures that each playthrough offers a fresh challenge, keeping gameplay exciting and dynamic.
  </p>

  <p><em>“Don’t kill the spider—it might just be your roommate.”</em> That’s where Webster was born.</p>
</section>

            <a href="https://github.com/MustafaBakir0/Interactive-p5-game-webster" class="github-link" target="_blank"><i class="fab fa-github"></i> View on GitHub</a>
        </div>
    </div>
</section>
     <!-- About section -->
        <section id="about" class="section">
            <h2>About Me</h2>
            <div class="about-content">
                <img src="assets/me.jpg" alt="Mustafa Bakir">
                <div class="bio">
                    <p>Hi there! I'm Mustafa Bakir, a multimedia designer and computer science enthusiast with a passion for blending technology and creativity. My journey has led me to work on diverse projects, from developing intricate data structures to crafting compelling visual narratives. This unique combination allows me to approach challenges with both analytical precision and artistic flair.  <br>
                        <br>

                    In the realm of project management, I thrive on orchestrating teams and resources to bring ideas to life. My focus on business strategies ensures that each project not only meets aesthetic and functional goals but also aligns with broader organizational objectives. I believe that effective design and technology solutions should drive tangible value and resonate with their intended audiences. <br>
                    <br>

                    Beyond my professional endeavors, I'm continually exploring the evolving intersections of art and technology. Whether it's experimenting with new design tools or delving into the latest in computer science research, I'm committed to lifelong learning and innovation. Let's connect and explore how we can create something remarkable together!</p>
                </div>
            </div>
        </section>

        <!-- Contact section -->
        <section id="contact" class="section">
            <h2>Get in Touch</h2>
            <div class="contact-content">
                <form id="contact-form" action="https://formspree.io/f/mqapynop" method="POST">
                    <input type="text" name="name" placeholder="Your Name" required>
                    <input type="email" name="email" placeholder="Your Email" required>
                    <textarea name="message" placeholder="Your Message" required></textarea>
                    <button type="submit">Send</button>
                </form>
            </div>
        </section>
    </div>

    <script src="sketch1.js"></script>
    <script src="script.js"></script>
</body>
</html>
